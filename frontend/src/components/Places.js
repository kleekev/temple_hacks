...
const Places = (props) => {
    ...
    const handleSelect = async (val) => {
        ...
        props.setEndAddress(results[0].formatted_address)
    }

    return (
        <Combobox onChange={handleSelect}>
            <Combobox.Input className="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0" value={value} onChange={e => setValue(e.target.value)} placeholder='Enter Destination'/>
